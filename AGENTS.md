...
T052 adds `steam_research.export.export_parquet`; install its optional dependency with `uv sync --locked --extra parquet`. It writes five datasets from SQLite in bounded row groups, with one deterministic file per dataset and a manifest; do not load the full review corpus into memory. T060 adds `steam_research.stage2` contracts only: payloads contain aggregate metric references and selected evidence, never raw reviews, and outputs must validate limits, lineage, positive denominators, confidence/counterevidence, and language-versus-geography constraints. Do not add provider calls or synthesis orchestration to T060.
