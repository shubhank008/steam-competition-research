...
T052 adds `steam_research.export.export_parquet`; install its optional dependency with `uv sync --locked --extra parquet`. It writes five datasets from SQLite in bounded row groups, with one deterministic file per dataset and a manifest; do not load the full review corpus into memory. T060 adds `steam_research.stage2` contracts. T061 adds `steam_research.synthesis`: payloads contain aggregate metric references and selected evidence, never raw reviews; outputs are post-validated for limits, lineage, positive denominators, confidence/counterevidence, and language-versus-geography constraints; migration-8 stores bounded payload/output lineage and resumable cost status. T062 adds deterministic `steam_research.reporting` Markdown rendering with hard word limits. T070 adds migration-9 bounded local diagnostics and safe status detail; T071 hardens fixture paths, provider endpoints, prompt boundaries, and adversarial tests. Do not add full CLI orchestration or live smoke to T061/T062.
...
- Migration-9 diagnostics are operational metadata only; keep them bounded and never add review text, prompts, response bodies, or profile identifiers.
- Offline fixtures are explicit regular local JSON files capped at 10 MB; reject symlinks and never enable fixture mode implicitly.
- Provider endpoints must be HTTPS without URL credentials, query strings, or fragments. Review content must not alter prompt instructions, taxonomy, or output schema.
...
## T072 scale benchmark

`tests/benchmarks/scale_benchmark.py` is an offline, reproducible harness that generates review records in bounded ingest batches, creates its SQLite database and optional Parquet output under a temporary directory, and emits JSON measurements. Run `uv run python tests/benchmarks/scale_benchmark.py --size 100000 --size 1000000 --seed-fraction 0.01 --no-export`; include export on 100k with `uv run python tests/benchmarks/scale_benchmark.py --size 100000 --seed-fraction 0.01`. `--seed-fraction` deterministically seeds at most 100k reviews with real eligibility decisions and successful Stage 1 classification lineage/aspects; it never invokes a provider. The harness measures database growth, batch upsert throughput, indexed grouping, aggregate/evidence runtime, local Stage 1 token estimation/batch planning, bounded Parquet export rows/time, and process peak RSS. It uses no network or provider credentials. Runtime artifacts are temporary and removed.

Recorded 2026-07-08 in the current container (Linux x86_64, 6 CPUs, 62 GiB RAM, Python 3.13.14, uv 0.12.0): a 1k seeded check ingested in 0.115s, aggregated in 0.012s, and selected evidence in 0.006s. The seeded 100k run used 86,786,048 bytes, ingested in 13.204s at 7,573.7 reviews/s, grouped in 0.121s, seeded 1,000 classifications, aggregated in 0.040s, selected 9 evidence rows in 0.025s, exported 100,000 reviews/1,000 classifications/1,000 aspects/102 metrics/1 quote in 1.798s, and peaked at 114.9 MiB RSS. Seeded 1M no-export used 902,270,976 bytes, ingested in 213.022s at 4,694.3 reviews/s, grouped in 1.300s, seeded 10,000 classifications, aggregated in 0.406s, selected 9 evidence rows in 0.246s, and peaked at 88.5 MiB RSS. One-million-row export was omitted because ingest already takes several minutes; 100k covers the bounded writer path. Batch planning remains capped at 100k (0.567s/0.574s), token estimation is local (0.817s/8.225s), and no index or transaction default changed. The generated corpus and seeded outputs are synthetic; provider latency/cost and full 1M export remain limitations.
