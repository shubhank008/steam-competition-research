...
`tests/benchmarks/scale_benchmark.py` is an offline, reproducible harness that generates review records in bounded ingest batches, creates its SQLite database and optional Parquet output under a temporary directory, and emits JSON measurements. Run `uv run python tests/benchmarks/scale_benchmark.py --size 100000 --size 1000000`; add `--no-export` for the faster 1M run. It measures database growth, batch upsert throughput, indexed grouping, aggregate invocation, local Stage 1 token estimation/batch planning, bounded Parquet export, and process peak RSS. It uses no network or provider credentials. The generated benchmark currently leaves eligibility/classification tables empty, so aggregate timing is the empty-population path and export classification/aspect datasets are empty; full provider execution is intentionally out of scope for T072.

Recorded 2026-07-08 in the current container (Linux x86_64, 6 CPUs, 62 GiB RAM, Python 3.13.14, uv 0.12.0): 100k reviews took 13.874s to ingest at 7,207.8 reviews/s, used 84,987,904 bytes, grouped in 0.119s, planned 5,000 batches for a 100k classification sample in 0.556s, estimated tokens in 0.802s, and exported reviews in 1.660s; peak RSS was 112.6 MiB. 1M reviews took 231.829s to ingest at 4,313.5 reviews/s, used 884,719,616 bytes, grouped in 1.358s, planned the same capped 100k classification sample in 0.593s, and estimated tokens for all 1M inputs in 8.276s; peak RSS was 61.7 MiB. No index or transaction default was changed because the measurements showed correctness and bounded batch behavior but did not provide a seeded eligible/classified comparison for tuning.
