...
- **Progress note (2026-07-08):** Added `tests/benchmarks/scale_benchmark.py` with deterministic bounded seeding (`--seed-fraction`, capped at 100k rows), eligibility decisions, successful Stage 1 lineage/aspects, aggregate/evidence timings, optional Parquet timing/row counts, and non-brittle regression coverage. Repeat with `uv run python tests/benchmarks/scale_benchmark.py --size 100000 --size 1000000 --seed-fraction 0.01 --no-export`; include export for 100k with `uv run python tests/benchmarks/scale_benchmark.py --size 100000 --seed-fraction 0.01`. In the current Linux x86_64 container (6 CPUs, 62 GiB RAM, Python 3.13.14, uv 0.12.0), the 1,000-row seeded check took 0.115s to ingest, 0.012s aggregate, and 0.006s evidence selection. The 100k run used 86,786,048 bytes, ingested in 13.204s (7,573.7 reviews/s), grouped in 0.121s, seeded 1,000 classified reviews, aggregated in 0.040s, selected 9 evidence rows in 0.025s, exported 100,000 reviews/1,000 classifications/1,000 aspects/102 aggregate metrics/1 quote in 1.798s, and held 114.9 MiB peak RSS. The 1M no-export run used 902,270,976 bytes, ingested in 213.022s (4,694.3 reviews/s), grouped in 1.300s, seeded 10,000 classified reviews, aggregated in 0.406s, selected 9 evidence rows in 0.246s, and held 88.5 MiB peak RSS; 1M export was omitted because the 1M ingest already takes several minutes and the 100k export covers the bounded writer path. Batch planning remains capped at a 100k sample (0.567s/0.574s), and token estimation remains local (0.817s/8.225s). Runtime artifacts were temporary and removed. Full pytest, mypy, and Ruff pass. No index or transaction default was changed; the generated corpus is synthetic and the seeded fraction is analytical coverage, not a provider execution benchmark. T072 remains `[~]` pending review/merge because acceptance is not considered complete on this working branch.
