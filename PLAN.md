...
- **Progress note (2026-07-08):** Added `tests/benchmarks/scale_benchmark.py` and a non-brittle harness regression check. Repeat with `uv run python tests/benchmarks/scale_benchmark.py --size 100000 --size 1000000 --no-export`; export can be included with `uv run uv --extra parquet run python ...` when PyArrow is available. In the current Linux x86_64 container (6 CPUs, 62 GiB RAM, Python 3.13.14), 100k used 84,987,904 bytes, ingested in 19.835s (5,041.5 reviews/s), grouped in 0.166s, and held peak RSS at 68.0 MiB. 1M used 884,719,616 bytes, ingested in 238.581s (4,191.4 reviews/s), grouped in 1.308s, estimated classification tokens in 8.122s, and held peak RSS at 61.7 MiB. Batch planning was capped at a 100k sample and took 0.722s/0.566s. Runtime artifacts were temporary and removed. The full suite (123 passed before adding the benchmark regression), mypy, Ruff, and focused harness tests pass. No index or transaction default was changed. T072 remains `[~]`: the generated corpus does not seed eligibility/classification lineage, so aggregate and classified/aspect export timings are empty-population paths; a seeded analytical comparison is still needed before claiming complete acceptance.
