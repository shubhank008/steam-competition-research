...
The first migration creates `projects` and `competitors`. A competitor is unique by `(project_id, appid)`, stores a canonical HTTPS Steam URL, and retains UTC ISO-8601 timestamps. Project and competitor services use parameterized SQL and keep path resolution outside the database layer. The current CLI surface is `init PATH`, the crawl/classify/aggregate/synthesize/run/export commands, and `status --project PATH [--json]`. T012 persists sanitized manifests, independent child units, and transactional domain-validated transitions. T013 adds review source lineage and canonical review facts.
